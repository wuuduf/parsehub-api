...
<dialog id="credentialDialog"><h2 id="credentialTitle">平台设置</h2><input id="platformId" type="hidden"><div id="cookieHint" class="muted" style="margin-bottom:12px;line-height:1.65"></div><label>Cookie</label><textarea id="cookie" placeholder="Cookie Header、JSON 或 key=value; ...；留空则保留原 Cookie"></textarea><br><label>解析代理（可选）</label><input id="proxy" placeholder="http://127.0.0.1:7890"><div class="row" style="margin-top:16px"><button onclick="saveCredential()">保存</button><button class="danger" onclick="clearCredential()">清除配置</button><button class="ghost" onclick="credentialDialog.close()">取消</button></div></dialog>
